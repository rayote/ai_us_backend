...
async def run_worker() -> None:
    ...
    definitions = MongoSurveyDefinitionRepository(
        database.database["survey_definitions"]
    )
    responses = MongoSurveyResponseRepository(database.database["survey_responses"])
    chat_submissions = MongoChatSubmissionRepository(
        database.database["chat_submissions"]
    )
    chat_download_artifacts = MongoChatDownloadArtifactRepository(
    ...
    async def selected_submissions(payload: dict[str, object]) -> list:
        submissions = await chat_submissions.list_submissions(
            str(payload.get("submissionPoint"))
            if payload.get("submissionPoint")
            else None
        )
        selected_ids = {
            str(value)
        ...
        if selected_ids:
            ...
            review_filter = payload.get("reviewFilter", "unreviewed")
            if review_filter == "unreviewed":
                ...
                submissions = [
                    ...
                    for submission in submissions
                    if submission.review is not None
                    and submission.review.status == review_filter
                ]
        ...
        if isinstance(school_level, str):
            ...
            submissions = [
                submission
                for submission in submissions
                if profiles.get(submission.participant_id) == school_level
            ]
        return submissions

    async def store_download_artifact(
        payload: dict[str, object], archive_path: Path, filename: str
    ) -> None:
        ...
        await chat_download_artifacts.create(
            new_artifact(
                str(payload["jobKey"]), file_id, filename, archive_path.stat().st_size
            )
        )
    ...
    async def build_chat_download(payload: dict[str, object]) -> None:
        ...
        try:
            count = await build_chat_archive(
                submissions, database.gridfs_bucket("chat_uploads"), archive_path
            )
            if count == 0:
                raise ValueError("선택한 제출에 다운로드할 첨부 파일이 없습니다.")
            await store_download_artifact(
                payload, archive_path, chat_download_filename()
            )
        finally:
    ...
    async def build_transcript_download(payload: dict[str, object]) -> None:
        ...
        try:
            ...
            await store_download_artifact(
                payload, archive_path, transcript_download_filename()
            )
        finally:
            archive_path.unlink(missing_ok=True)
    ...
    worker = QueueWorker(
        ...
        {
            "survey_response": lambda payload: store_survey_response(
                payload, definitions, responses
            ),
            "chat_submission": lambda payload: store_chat_submission(
                payload, chat_submissions
            ),
