...
def _submission_service(request: Request) -> ChatSubmissionService:
    ...
    if participants is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="대화문 제출 서비스를 준비 중입니다.")
...
async def submit_chat(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="유효한 대화 공유 링크가 아닙니다.") from error
