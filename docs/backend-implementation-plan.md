...
- 비밀번호는 평문 또는 초기 비밀번호 값으로 저장하지 않고 단방향 해시로 저장
- 첫 로그인 후 비밀번호 변경 강제
- `PasswordReset.sendResetLink`는 1회용 재설정 토큰을 동기 생성하고, 이메일 발송 작업은 Queue로 처리한다. 이메일 발송 지연·재시도가 로그인 화면을 막지 않도록 한다.
...
- 작업자는 `queued`, `processing`, `completed`, `failed` 상태와 재시도 횟수를 관리한다. 중복 제출 식별자를 고유 인덱스로 두어 같은 최종 제출이 여러 번 저장되지 않게 한다.
...
| 비밀번호 재설정 이메일 | MongoDB 영속 Queue | 외부 메일 서비스 지연·실패 격리 |
...
- `email_jobs`: 비밀번호 재설정 등 이메일 발송 대기열. 수신 대상, 템플릿 유형, 상태, 재시도 횟수, 오류 사유, 생성/처리 시각을 저장한다.
...
- 승인 후 초기 비밀번호 전달 방법과 첫 로그인 비밀번호 변경 화면
- 비밀번호 재설정 이메일 발송에 사용할 기관 메일 계정 또는 서비스
- 연구자 계정 생성·권한 부여 절차
